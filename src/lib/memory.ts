...
export async function ensureThread(opts: { project_id: string; thread_id: string }) {
  ...
  if (!data?.id) {
    ...
    if (insertErr) throw new Error(`ensureThread failed: ${insertErr.message}`);
  }
}
...
export async function appendMessage(project_id: string, thread_id: string, role: ChatRole, content: string) {
